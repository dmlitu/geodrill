...
- **DWG parsing**: no Python library reads binary DWG directly. `dwg_converter.py` shells out to an external converter (`dwg2dxf` from GNU LibreDWG, or a proprietary `ODAFileConverter`), resolved via `GEODRILL_DWG_CONVERTER` env var or `$PATH`. No hard-coded paths, no `shell=True`, upload bytes never touch a shell string. LibreDWG is **not packaged for Debian/Ubuntu** (checked directly against packages.debian.org — no such package) — the production Docker image builds `dwg2dxf` from the official GNU source tarball instead (see Deployment). Locally: `brew install libredwg` on macOS; on Linux, build from `https://ftp.gnu.org/gnu/libredwg/` the same way `backend/Dockerfile` does.
...
- **Health check**: `modules/cad/health.py` → `check_cad_environment()` logs the CAD subsystem's status once at startup (`CAD subsystem: DXF parser: OK / DWG converter: OK (path)`) and is surfaced live in `GET /health`'s `cad` field — a missing converter degrades the DWG-upload feature but never flips overall `/health` status, so it doesn't take deploys down.
- **Performance**: `CadAnalyzer.analyze()` logs one structured "CAD analyze timing" line per request (per-stage ms breakdown) — read it first when a production upload is slow. Profiled on the real fixtures: DXF parsing (`ezdxf` loading the full document) dominates, not the DWG->DXF conversion subprocess. `routers/cad.py` runs the whole analyze/inspect call in Starlette's thread pool (`run_in_threadpool`) so a slow upload never blocks the event loop / other concurrent requests, with an outer `asyncio.wait_for` safety net (`GEODRILL_CAD_PROCESSING_TIMEOUT`, default 180s) — verified with real concurrent traffic during this change. The frontend (`api.js`) uses a longer, CAD-specific fetch timeout (120s, not the global 30s default) since a cold Render free-tier instance alone can take ~60s to spin up before a request is even received.
...
- **Backend**: Render, **Docker runtime** (`render.yaml`, `runtime: docker`, `dockerfilePath: backend/Dockerfile`). Not Render's native Python runtime — that has no apt/system-package access, and CAD analysis needs `dwg2dxf` (GNU LibreDWG), a compiled binary pip can't install. `backend/Dockerfile` builds `dwg2dxf` from the official GNU source release (pinned version + SHA-256) in a builder stage, bakes just the binary into the final image, and sets `GEODRILL_DWG_CONVERTER` automatically — DWG upload works with zero manual server setup. Verified end-to-end (real Docker build + real DWG upload over HTTP) during this change; see git history around `security(cad)` / `fix(cad)` commits.
